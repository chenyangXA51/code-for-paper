...
class DCN():
    ...
    def create_model(self, ):
        ...
        if len(self.hidden_size) > 0 and self.cross_num > 0:  # Deep & Cross
            ...
            x = Dense(20, activation='relu')(stack_out)
...
if __name__ == "__main__":
    model = DCN({"sparse":sparse_feature_dim,'dense':{'price':1}}, cross_num=2, hidden_size=[32, ], keep_prob=0.5,
                ).model
    ...
    df_train['param_1'].fillna(value='_NA_', inplace=True)
    df_train['param_2'].fillna(value='_NA_', inplace=True)
    df_train['param_3'].fillna(value='_NA_', inplace=True)

    df_train['description'].fillna(value='_NA_', inplace=True)

    df_train['activation_date'] = pd.to_datetime(df_train['activation_date']).dt.weekday.astype(np.int32).values
    #tr_week = np.expand_dims(tr_week, axis=-1)

    eps = 1e-10
    df_train['price'] = np.log(df_train['price']+eps)
    df_train['price'][df_train['price'].isnull()] = -1.
    data=df_train
    price=np.array(df_train['price'])
    import pandas as pd
    from sklearn.preprocessing import LabelEncoder

    sparse_features = ['region','city','parent_category_name','param_1','param_2','param_3','user_type','activation_date',]
    dense=['price']
    ...
    model_input.append(price)

    model.fit(model_input,data[target],batch_size=4096,epochs=1000,verbose=1,validation_split=0.2)
